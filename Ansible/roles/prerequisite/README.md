...
When installing from the local repo, this role preseeds persistent firewall
package prompts, sets `DEBIAN_FRONTEND=noninteractive`, and uses `dpkg -i` on
every `.deb` under `ANSIBLE_PREREQUISITE_REPO_DEST`.
