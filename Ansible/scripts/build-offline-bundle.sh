...
ANSIBLE_OFFLINE_BUNDLE_BUILD_ALL="${ANSIBLE_OFFLINE_BUNDLE_BUILD_ALL:-true}"
ANSIBLE_DNS_TIME_SERVICES_BUILD_IMAGES="${ANSIBLE_DNS_TIME_SERVICES_BUILD_IMAGES:-${ANSIBLE_OFFLINE_BUNDLE_BUILD_ALL}}"
ANSIBLE_DNS_SERVER_BUILD_IMAGE="${ANSIBLE_DNS_SERVER_BUILD_IMAGE:-${ANSIBLE_DNS_TIME_SERVICES_BUILD_IMAGES}}"
ANSIBLE_TIME_SERVER_BUILD_IMAGE="${ANSIBLE_TIME_SERVER_BUILD_IMAGE:-${ANSIBLE_DNS_TIME_SERVICES_BUILD_IMAGES}}"
...
ANSIBLE_PREREQUISITE_DOWNLOAD_PACKAGES="${ANSIBLE_PREREQUISITE_DOWNLOAD_PACKAGES:-${ANSIBLE_OFFLINE_BUNDLE_BUILD_ALL}}"
...
ANSIBLE_PACKAGE_UPDATE_OFFLINE_PACKAGES="${ANSIBLE_PACKAGE_UPDATE_OFFLINE_PACKAGES:-vim=2:9.1.0016-1ubuntu7.15 vim-common=2:9.1.0016-1ubuntu7.15 vim-runtime=2:9.1.0016-1ubuntu7.15 vim-tiny=2:9.1.0016-1ubuntu7.15 xxd=2:9.1.0016-1ubuntu7.15 python3-wheel=0.42.0-2ubuntu0.1~esm1}"
...
ANSIBLE_ZABBIX_SERVER_DOWNLOAD_PACKAGES="${ANSIBLE_ZABBIX_SERVER_DOWNLOAD_PACKAGES:-${ANSIBLE_OFFLINE_BUNDLE_BUILD_ALL}}"
  ...
  docker run --rm \
    ...
    bash -lc '
      ...
      apt-get install -y --download-only --no-install-recommends \
        ...
        ${OFFLINE_PACKAGES}
      for spec in ${OFFLINE_PACKAGES}; do
        package="${spec%%=*}"
        version=""
        if [[ "${spec}" == *=* ]]; then
          version="${spec#*=}"
        ...
        if [[ -z "${found}" ]]; then
          if [[ -n "${version}" ]]; then
            echo "Missing downloaded package ${package}=${version} in /offline-debs" >&2
          else
            echo "Missing downloaded package ${package} in /offline-debs" >&2
          fi
          exit 1
        fi
      done
...
if [[ "${ANSIBLE_PACKAGE_UPDATE_DOWNLOAD_PACKAGES}" == "true" ]]; then
  download_ubuntu_packages \
...
if [[ "${ANSIBLE_DNS_SERVER_BUILD_IMAGE}" == "true" ]]; then
  build_service_image \
    "DNS" \
...
if [[ "${ANSIBLE_TIME_SERVER_BUILD_IMAGE}" == "true" ]]; then
  build_service_image \
    "time" \
