...
load_ansible_env() {
  ...
  env_file_for_tag() {
    local tag="$1"
    case "${tag}" in
      base|prerequisite|docker|iptable|iptables|iptables_primary|iptables_zabbix|iptable_block|iptables_block|mariadb_remove)
        printf '%s\n' "20-base.env"
        ;;
      hostname|network)
        printf '%s\n' "25-host-network.env"
      ...
      external_disk)
        ...
        ;;
      docker_swarm|docker_swarm_iptables|docker_swarm_iptables_nodes|docker_swarm_iptables_allow_connect_in|docker_swarm_iptables_allow_connect_out|docker_swarm_iptables_docker_user_drop|docker_swarm_labels|docker_swarm_reset|docker_swarm_leave)
        printf '%s\n' "60-docker-swarm.env"
        ;;
      tldh_database)
