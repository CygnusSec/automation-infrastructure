...
| [User Password](user-password/README.md) | Change a local Linux user password on selected hosts. | `./scripts/run-ansible.sh deploy --tags user_password` |
| [Logrotate Size Limit](logrotate-size-limit/README.md) | Delete matching Ubuntu log files when they reach the configured size. | `./scripts/run-ansible.sh deploy --tags logrotate_size_limit` |
